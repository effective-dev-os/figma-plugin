...
- *(more skills added as patterns prove out: `/implementor`, `/post-feature`, `/audit`, `/diagnose`, `/refactor`, `/defrag`.)*

Other bundled skills under `.claude/skills/` (see `.memory-bank/tech-details/dependencies.md`):
- `/setup` — installs the harness into a target project (used by README's install path)
- `anti-ai-slop-writing`, `frontend-design`, `swiftui-pro`, `swiftui-macos-26`, `memory-bank-defrag` — support skills loaded by exec / consilium agents
