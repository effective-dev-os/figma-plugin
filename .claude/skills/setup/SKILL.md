...
- **Already installed.** If `<target>/.harness-lock` exists with same commit SHA → emit "Already installed at this version. Use the future `/sync` skill to update."
- **Older harness install.** If `.harness-lock` exists with a different SHA → refuse `/setup`; tell the user this is an update scenario (Phase 2 — `harness sync` not yet shipped).
...
- Does not auto-update later. That's `/sync` (OQ-002, Phase 2 — not yet shipped).
